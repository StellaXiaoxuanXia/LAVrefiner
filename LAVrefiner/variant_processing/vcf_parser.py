...
def count_overlapping_variants(groups) -> int:
    """Count members of overlapping groups, excluding SNP/pass-through groups."""
    return sum(len(group.variants) for group in groups)
...
def process_variants(config) -> List[VariantGroup]:
    ...
    overlapping_count = count_overlapping_variants(multi_group)
    non_snp_count = variant_count - snp_count
    percentage_lav_overlapped = (
        100.0 * overlapping_count / non_snp_count if non_snp_count else 0.0
    )
